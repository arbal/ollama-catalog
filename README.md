...
### Git-as-Time-Series
The committed catalog uses git-friendly split artifacts:

- `out/models.jsonl` — stable model records and descriptive metadata
- `out/pulls.jsonl` — volatile pull-count records
- `out/metadata.json` — scrape timestamp and model count
- `out/seen_slugs.json` and `out/discovered_slugs.json` — discovery state

`out/ollama_catalog.json` is an ignored local aggregate derived from those
committed files. Run `python3 scripts/render_catalog.py` when a single-file
catalog is needed locally. When deployed on a daily cron schedule via GitHub
Actions (or the provided `scripts/daily-run.sh`), the history of the committed
split artifacts becomes a queryable time-series database.
...
The ignored local aggregate generated by `scripts/render_catalog.py` has the
following schema. The committed source of truth is the split-artifact set
described above.
```json
{
