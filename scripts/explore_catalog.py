...
def show_catalog_history(max_commits: int = 30, fmt: str | None = None):
    ...
    if not entries:
        if fmt == "json":
            ...
            console.print(Panel("[yellow]No pull history found.[/yellow]", title="Empty Result", border_style="yellow"))
        return
    ...
    if not ns_models:
        ...
        if not ns_models:
            if fmt == "json":
                ...
                console.print(Panel(f"[yellow]No models found for namespace '{ns}'[/yellow]", title="Empty Result", border_style="yellow"))
            return
...
def show_detail(models, slug, all_models=None, enrich=False):
    ...
    if not matches:
        console.print(Panel(f"[yellow]No model found matching '{slug}'[/yellow]", title="Empty Result", border_style="yellow"))
        return
    if len(matches) > 1:
...
def main():
  ...
  %(prog)s --format tsv --stale 365                # stale models for spreadsheet/scripting""",
    ...
    if args.history:
        ...
        if not entries:
            console.print(Panel(f"[yellow]No history found for '{args.history}'.[/yellow]", title="Empty Result", border_style="yellow"))
            return
    ...
    if args.namespace_stats:
        if args.namespace_stats not in (True, "__from_namespace__"):
            ...
            if not rows:
                console.print(Panel("[yellow]No namespaces available for --namespace-stats.[/yellow]", title="Empty Result", border_style="yellow"))
                return
