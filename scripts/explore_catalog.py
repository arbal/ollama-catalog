...
def show_list(models, limit, new_days=None, sort_field="pulls", filter_parts=None, total_catalog=None, compare_ref="", vram_gb=None):
    if not models:
        filter_str = ", ".join(filter_parts) if filter_parts else "current filters"
        console.print(Panel(
            f"[dim]No models found matching:[/dim] {filter_str}\n\n[italic]Try broadening your search criteria or removing some filters.[/italic]",
            title="No Results",
            border_style="yellow"
        ))
        return
